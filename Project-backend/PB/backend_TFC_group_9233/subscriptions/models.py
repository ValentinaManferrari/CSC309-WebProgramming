...
class Subscription(models.Model):
    ...
    title = models.SlugField(null=False,unique=True)
    subscription_type = models.CharField(choices=SUBSCRIPTION_CHOICES, default = None, max_length = 50)
    price = models.FloatField(default=0)
    iteration = models.DurationField(verbose_name='iteration(days,hours,min,sec)', default=datetime.timedelta(days=30), editable=False)
...
class UserSubscription(models.Model):
    ...
    user = models.ForeignKey(OurUser, related_name = 'user_subscription', on_delete=models.CASCADE)
    plan = models.ForeignKey(Subscription, related_name='plan_subscription', on_delete=models.CASCADE, null=True)
    start_date = models.DateField(null=True, blank=True)
    next_payment_date = models.DateField(null=True, blank=True, editable=False)
    # first_renewal = models.BooleanField(null=False, editable=False, default=True)
...
class Card(models.Model):
    ...
    expire_date = models.DateField(validators=[validate_expire_date], null=False)
...
class Payment(models.Model):
    ...
    recurrence_type = models.CharField(null=True, choices=SUBSCRIPTION_CHOICES, blank=True, max_length=10)
    recurrence = models.BooleanField(null=False, blank=False, default=False) # if a new payment need to be added once this payment has been done
    # success = models.BooleanField(null=False, blank=False, default=False)
    status = models.CharField(null=False,blank=False, choices= PAYMENT_STATUS, max_length=10)
